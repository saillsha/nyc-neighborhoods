...
$(document).ready(function(){
    ...
    $('label.line').hover(
        function(){
            polygons[$(this).find('span').text()].setOptions({fillOpacity: .8});
        },
        function(){
            polygons[$(this).find('span').text()].setOptions({fillOpacity: .35});
        }
    );
    var select = $('#info-window select');
    $.each(colors, function(color, hexValue){
        $(select).append('<option>'+color+'</option>');
    });
});
